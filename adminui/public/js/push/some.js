...
initUiValue = function () {
    ...
    $.ajax({
        type: "POST",
        url: GlobalRootPath + "/push/getTags",
        data: JSON.stringify({}),
        ...
        success: function (resData) {
            ...
            if (resData.data.length > 0) {
                ...
                for (var i = 0; i < resData.data.length; i++) {
                    optHtml += "<option value=\"" + resData.data[i] + "\">"
                        + resData.data[i] + "</option>";
                }
                $("#tags").append(optHtml);
                $("#tagsAnd").append(optHtml);
                $("#tags").multiselect('rebuild');
                $("#tagsAnd").multiselect('rebuild');
            }
        },
...
validateData = function () {
    ...
    GlobalParam.tags = $("#tags").val();
    GlobalParam.tagsAnd = $("#tagsAnd").val();
    GlobalParam.alias = $("#alias").val();
    GlobalParam.registerIds = $("#registerIds").val();
    switch ($("#platform").val()) {
        case "all":
            GlobalParam.toAndroid = true;
            GlobalParam.toIOS = true;
...
send2Some = function () {
    ...
    $.ajax({
        type: "POST",
        url: GlobalRootPath + "/push/sendToSome",
        ...
        success: function (resData) {
            ...
            if (resData.code !== "0") {
                ...
                $("#myAlert").html("<strong>出现错误</strong>：" + JSON.stringify(resData.msg));
